/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   conv.c                                             :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: lucinguy <lucinguy@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/04/29 18:15:00 by lucinguy          #+#    #+#             */
/*   Updated: 2026/04/29 18:06:32 by lucinguy         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_fprintf.h"

int	ft_fprintf_putnbr_t_fd(long n, int fd)
{
	int	count;

	count = 0;
	if (n < 0)
	{
		count += ft_fprintf_putchar_fd_pf('-', fd);
		n = -n;
	}
	if (n >= 10)
		count += ft_fprintf_putnbr_t_fd(n / 10, fd);
	ft_fprintf_putchar_fd_pf(n % 10 + 48, fd);
	return (count + 1);
}

int	ft_fprintf_putnbr_base_x(unsigned long n, int uppercase, int fd)
{
	char	*base;
	int		count;

	count = 0;
	if (uppercase)
		base = "0123456789ABCDEF";
	else
		base = "0123456789abcdef";
	if (n >= 16)
		count += ft_fprintf_putnbr_base_x(n / 16, uppercase, fd);
	write(fd, &base[n % 16], 1);
	return (count + 1);
}

int	ft_fprintf_putpointer(unsigned long n, int fd)
{
	int	count;

	count = 0;
	if (n == 0)
	{
		ft_fprintf_putstr_fd_pf("(nil)", fd);
		return (5);
	}
	ft_fprintf_putstr_fd_pf("0x", fd);
	count += ft_fprintf_putnbr_base_x(n, 0, fd);
	return (count + 2);
}
